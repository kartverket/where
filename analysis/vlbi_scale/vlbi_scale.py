...
dset_ts = dataset.Dataset.read(rundate=date(1970,1,1), pipeline=pipeline, stage="timeseries", label="0", session="", id=dset_id)
...
plt.scatter(dset_ts.time.utc.datetime[idx_2], dset_ts.neq_helmert_scale[idx_2], c=dset_ts.network_volume[idx_2], marker=".")
...
for f, u in zip(fields,units):
    ...
    plt.figure(figsize=(12,8), dpi=150)
    plt.scatter(dset_ts.time[idx_2].utc.datetime, dset_ts[f][idx_2], c=dset_ts.network_volume[idx_2], marker=".")
    cbar = plt.colorbar()
    cbar.set_label(f"Network volume [Mm^3]")
    plt.title(f"{name}")
    plt.ylabel(f"{name} [{u}]")
...
# TODO: sort dset_ts.time[idx_2] and dset_ts.neq_helmert_scale[idx]

for i, t in enumerate(dset_ts.time[idx_2].utc.datetime):
    ...
    idx_time_lower = dset_ts.time[idx_2].utc.datetime > time_lower_limit
    idx_time_upper = dset_ts.time[idx_2].utc.datetime < time_upper_limit
    ...
    running_median[i] = np.median(dset_ts.neq_helmert_scale[idx_2][idx_interval])

plt.figure(figsize=(12,8), dpi=150)
plt.plot(dset_ts.time[idx_2].utc.datetime, running_median, color="red")
plt.scatter(dset_ts.time[idx_2].utc.datetime, dset_ts.neq_helmert_scale[idx_2], c=dset_ts.network_volume[idx_2], marker=".")
cbar = plt.colorbar()
cbar.set_label(f"Network volume [Mm^3]")
...
if make_csv:
    ...
    with open(filename, "w") as fid:
        header = "date, dbc, scale [ppb], t_x [m], t_y [m], t_z [m], r_1 [mas], r_2 [mas], r_3 [mas], volume [Mm^3]"
        fid.write(f"{header}\n")
        rundate = dset_ts.rundate[idx_2]
        sort_idx = np.argsort(rundate)
        rundate = rundate[sort_idx]
        session = dset_ts.session[idx_2][sort_idx]
        scale = dset_ts.neq_helmert_scale[idx_2][sort_idx]
        t_x = dset_ts.neq_helmert_T_X[idx_2][sort_idx]
        t_y = dset_ts.neq_helmert_T_Y[idx_2][sort_idx]
        t_z = dset_ts.neq_helmert_T_Z[idx_2][sort_idx]
        r_1 = dset_ts.neq_helmert_alpha[idx_2][sort_idx]
        r_2 = dset_ts.neq_helmert_beta[idx_2][sort_idx]
        r_3 = dset_ts.neq_helmert_gamma[idx_2][sort_idx]
        volume = dset_ts.network_volume[idx_2][sort_idx]
        for rd, dbc, s, tx, ty, tz, r1, r2, r3, v in zip(rundate, session, scale, t_x, t_y, t_z, r_1, r_2, r_3, volume):
            fid.write(f"{rd}, {dbc}, {s}, {tx}, {ty}, {tz}, {r1}, {r2}, {r3}, {v} \n")
