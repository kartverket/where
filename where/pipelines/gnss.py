...
def get_args(rundate: datetime.date, input_args: Union[None, List[str]]=None) -> Set[str]:
...
def read(stage: str, dset: "Dataset") -> None:
   ...
   | time                 | Time              | Observation time given as Time object                                 |
    ...
    dset.meta.add("site_id", dset.meta["marker_name"].upper(), section=station)
    dset.add_float(
    ...
    # Select and clean GNSS observation to process
...
def calculate_estimate(stage: str, dset: "Dataset") -> None:
    ...
    for iter_num in itertools.count(start=1):
        ...
        if "observed" in dset.fields:
            ...
            dset.add_float("observed", val=gnss.get_observation(dset), unit="meter", write_level="analysis")

        # Get model corrections
        if "calc" in dset.fields:
            dset.calc[:] = delta_delay
        else:
            dset.add_float("calc", val=delta_delay, unit="meter", write_level="analysis")

        if "residual" in dset.fields:
            dset.residual[:] = dset.observed - dset.calc
            dset.residual_prefit[:] = dset.residual
        else:
            dset.add_float("residual", val=dset.observed - dset.calc, unit="meter", write_level="operational")
            dset.add_float("residual_prefit", val=dset.residual, unit="meter", write_level="analysis")
