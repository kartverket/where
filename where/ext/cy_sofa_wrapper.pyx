...
$Revision: 13086 $
$Date: 2017-08-23 19:04:16 +0200 (on., 23 aug. 2017) $
...
def vectorized_s06(time, x, y):
    ...
    return c_s06(time.tt.jd1, time.tt.jd2, x, y)
    #return np.array([sofa.iau_s06(t.jd1, t.jd2, x_i, y_i) for t, x_i, y_i in zip(time.tt, x, y)])

@cython.boundscheck(False)
@cython.wraparound(False)
cdef double[:] c_s06(double[:] jd1, double[:] jd2, double[:] x, double[:] y):
    cdef int size = jd1.shape[0]
    cdef double[:] s = np.empty(size, dtype=np.double)

    for i in range(size):
        s[i] = sofa.iau_s06(jd1[i], jd2[i], x[i], y[i])
    return s
