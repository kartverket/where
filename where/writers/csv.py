...
@plugins.register
def csv(dset):
    """Write a list of calculated observations in the CSV format.

    Args:
        dset:  Dataset, data for a model run.
    """
    row_number=config.tech.csv.row_number.bool
    csv_path=files.path("output_csv")
    dset.as_dataframe().to_csv(csv_path,index=row_number)
