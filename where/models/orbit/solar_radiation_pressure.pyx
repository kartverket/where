...
def solar_setup(rundate, force_parameters, sat_name, time_grid, epochs, body_pos_gcrs, body_pos_itrs, bodies, gcrs2itrs):
    """
    Args:
...
def solar_radiation_pressure(double[:] sat_pos_gcrs, str sat_name, int num_param, int current_step, **_not_used):
    ...
    # sun_flux[current_step] = 1367
    # Acceleration of satellite due to solar radiation pressure
    # Equation (3.75) in Montenbruck [1]
    cdef double[:] acc = np.zeros(3)
    for i in range(0, 3):
        acc[i] = (-radiation_pressure_coefficient * (sun_flux[current_step] / c) * (satellite.area / satellite.mass) * sat_sun_vec[i] / sat_sun_norm)
    ...
    for i in range(0, 3):
        for j in range(0, 3): 
            trans[i, j] = (radiation_pressure_coefficient * (sun_flux[current_step] / c) * (satellite.area / satellite.mass) / sat_sun_norm *
             ((i == j) - 3 * sat_sun_vec[i] * sat_sun_vec[j] / sat_sun_norm**2))
    ...
    shadow_area = (sun_rad**2 * libc.math.acos(x / sun_rad) + earth_rad**2 * libc.math.acos((sun_earth_sep - x) / earth_rad)
                   - sun_earth_sep * y)
