...
def write_to_dataset(dset, rundate=None, session=None, obs_format=None, **obs_args):
    ...
    file_vars1 = config.create_file_vars(rundate, TECH, session=session, **obs_args)
    last_date_to_read = rundate + timedelta(days=config.tech.arc_length.int + 1)
    parser1 = parsers.parse_key(f"slr_obs_{obs_format}", file_vars1)
    file_vars2 = config.create_file_vars(last_date_to_read, TECH, session=session, **obs_args)
    parser2 = parsers.parse_key(f"slr_obs_{obs_format}", file_vars2)

    if parser1.data_available and parser2.data_available:
        data = _write_to_dataset(parser1, parser2, dset, rundate, session)
        _write_met_to_dataset(dset, data, rundate, session)
...
def _write_to_dataset(parser1, parser2, dset, rundate, session):
    ...
    dset.add_time("time", val=time, scale="utc")
    dset.add_text(val=obs_data["meta"]["station"], time="time", fieldnames="station")
    dset.add_text(val=obs_data["meta"]["satellite"], time="time", fieldnames="satellite")
    dset.add_float(val=obs_data["meta"]["bin_rms"], time="time", unit="picoseconds", fieldnames="bin_rms")
    # Positions
    ...
    dset.add_position(
        "site_pos", time="time", itrs=np.array([obs_data["pos_" + s][idx] for idx, s in enumerate(dset.station)])
    )
    # Station data
...
def _write_met_to_dataset(dset, data, rundate, session):
