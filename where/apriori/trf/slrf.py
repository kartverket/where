"""A parser for reading data from SLRF files in SNX format
...
class Slrf(trf.TrfFactory):
    ...
    file_key_pattern = "trf-slrf_{}"

    def __init__(self, time, version=None):
        """Set up a new Terrestrial Reference Frame object based on the SLRF Sinex files
        ...
        if self.solution == "last":
            candidates = config.files.glob_variable(self.file_key_pattern.format(self.format), "version", r".*")
            try:
    ...
    @property
    def file_paths(self):
        """File paths used to read SLRF data (dependent on format)
